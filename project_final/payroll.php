        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>R<?php echo number_format($employee['grossSalary'], 2); ?></td>
                    ...
                    <td>
                        <button class="btn btn-sm btn-outline-dark" data-bs-toggle="modal" 
                                ...
                                data-gross="<?php echo number_format($employee['grossSalary'], 2); ?>"
                        ...
                        <button class="btn btn-sm btn-outline-dark" data-bs-toggle="modal" 
                                ...
                                data-id="<?php echo $employee['employeeId']; ?>"
                                data-name="<?php echo $employee['name']; ?>"
                                data-hours="<?php echo $employee['hoursWorked']; ?>"
                                data-leave="<?php echo $employee['leaveDeductions']; ?>"
                                data-salary="<?php echo $employee['grossSalary']; ?>">
...
<div class="modal fade" id="editPayrollModal" tabindex="-1" aria-labelledby="editPayrollModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editPayrollModalLabel">Edit Payroll Data</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="update_payroll.php" method="POST">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="editHoursWorked" class="form-label">Hours Worked</label>
                        <input type="number" class="form-control" id="editHoursWorked" name="hours_worked" required>
                    </div>
                    <div class="mb-3">
                        <label for="editLeaveDeductions" class="form-label">Leave Days</label>
                        <input type="number" step="0.5" class="form-control" id="editLeaveDeductions" name="leave_deductions" required>
                    </div>
                    <div class="mb-3">
                        <label for="editGrossSalary" class="form-label">Gross Salary (R)</label>
                        <input type="number" step="0.01" class="form-control" id="editGrossSalary" name="gross_salary" required>
                    </div>
                </div>
                <div class="modal-footer">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var editModal = document.getElementById('editPayrollModal');
    editModal.addEventListener('show.bs.modal', function(event) {
        var button = event.relatedTarget;
        document.getElementById('editEmployeeId').value = button.getAttribute('data-id');
        document.getElementById('editEmployeeName').value = button.getAttribute('data-name');
        document.getElementById('editHoursWorked').value = button.getAttribute('data-hours');
        document.getElementById('editLeaveDeductions').value = button.getAttribute('data-leave');
        document.getElementById('editGrossSalary').value = button.getAttribute('data-salary');
    });
});
